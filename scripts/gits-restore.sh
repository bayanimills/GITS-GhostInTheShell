...
Examples:
  ...
  gits-restore.sh --component agents --item agentname --from 2026-03-22_1430
...
list_component_contents() {
    ...
    # Tarball paths look like: .openclaw/agents/agentname/... or .openclaw/openclaw.json
...
# e.g. restore just "agentname" from agents.tar.gz
restore_item() {
    local component="$1"
    local item="$2"
...
restore_legacy_snapshot() {
    ...
    local backup_name=""
    if [ -d "$OPENCLAW_ROOT" ]; then
        backup_name="${OPENCLAW_ROOT}.backup-$(date +%Y%m%d-%H%M%S)"
        log_message "Backing up existing installation to $backup_name"
        mv "$OPENCLAW_ROOT" "$backup_name"
    fi
